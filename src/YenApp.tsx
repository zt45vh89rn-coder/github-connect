  ...
  const renderHub = () => {
    ...
    return (
      <div style={{padding:16}}>
        ...
        {/* Monthly goal */}
        {monthlyGoal>0&&(
          <div className="yen-card" style={{...card,padding:"16px 18px",marginBottom:14}}>
            ...
            <div style={{height:12,background:"#E8EEFF",borderRadius:8,overflow:"hidden",marginBottom:8,boxShadow:"inset 0 2px 4px rgba(59,130,246,0.1)"}}>
              <div style={{height:"100%",width:`${goalPct}%`,background:goalPct>=100?"linear-gradient(90deg,#10B981,#34D399)":"linear-gradient(90deg,#3B82F6,#8B5CF6)",borderRadius:8,transition:"width .8s cubic-bezier(.34,1.56,.64,1)",boxShadow:goalPct>0?"0 2px 8px rgba(59,130,246,0.4)":"none"}}/>
            </div>
            <div style={{display:"flex",justifyContent:"space-between"}}>
              <div style={{fontSize:13,color:C.gr,fontFamily:M,fontWeight:800}}>{fmtK(totalRev)}</div>
              <div style={{fontSize:12,color:C.t3,fontFamily:M}}>目標 {fmtK(monthlyGoal)}</div>
            </div>
          </div>
        )}
          ...
          {[
            ...
            <div key={i} className="yen-card yen-pop" style={{...card,padding:"16px 14px",background:k.bg,border:`2px solid ${k.c}22`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                <div style={{fontSize:11,color:k.c,fontWeight:800,letterSpacing:0.3}}>{k.l}</div>
                <span style={{fontSize:18}}>{k.emoji}</span>
              </div>
              <div style={{fontSize:22,fontWeight:900,color:k.c,fontFamily:M,marginBottom:4}}>{k.v}</div>
              <div style={{fontSize:11,color:k.c+"99",fontWeight:700}}>{k.d}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:18}}>
          {[
            ...
            <button key={i} onClick={a.fn} className="yen-card yen-pop yen-btn3d" style={{...card,padding:"16px 10px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:8,borderRadius:22,border:`2px solid ${a.color}22`,background:`linear-gradient(160deg,#fff,${a.color}0A)`}}>
              ...
              <span style={{fontSize:11,color:a.color,fontWeight:800}}>{a.l}</span>
            </button>
          ))}
        </div>
        {/* Urgent tasks */}
